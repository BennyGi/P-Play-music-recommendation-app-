    ...
    if (playlist?.tracks && playlist.tracks.length > 0) {
      ...
      playlist.tracks.forEach((t) => {
        if (t.artistId && !artistMap.has(t.artistId)) {
          artistMap.set(t.artistId, { id: t.artistId, name: t.artist, image: t.image || null });
        }
      });
              ...
              {suggestedTracks.map((track) => (
                <div
                  ...
                  className={`flex items-center justify-between p-4 rounded-lg transition-colors ${
                    currentTrack?.id === track.id ? 'bg-white/10 border border-purple-500/30' : 'bg-white/5 hover:bg-white/10'
                  }`}
                >
                  <div className="flex items-center gap-4 overflow-hidden cursor-pointer flex-1" onClick={() => setCurrentTrack(track)}>
                    ...
                    <div className="min-w-0">
                      <p className={`font-semibold text-lg truncate ${currentTrack?.id === track.id ? 'text-purple-300' : 'text-white'}`}>
                        {track.title}
                      </p>
              ...
              <div className="flex flex-wrap gap-3">
                {selectedGenreNames.map((genre) => {
                  ...
                  return (
                    <button
                      key={key}
                      onClick={() => (isBlocked ? handleUnblock(key) : handleBlockGenre(genre))}
