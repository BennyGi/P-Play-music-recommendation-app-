...
import { Check, SkipForward, Users, Loader, Sparkles, AlertCircle, RefreshCw, ArrowLeft } from 'lucide-react';
...
const ArtistSelection = ({
  ...
  const [selectedArtistIds, setSelectedArtistIds] = useState(() =>
    (initialSelected || []).map(a => a?.id).filter(Boolean)
  );
  ...
  useEffect(() => {
    setSelectedArtistIds((initialSelected || []).map(a => a?.id).filter(Boolean));
  }, [initialSelected]);
  ...
  const getArtistTooltip = (artist) => {
    ...
    if (artist.genre) criteria.push(`${artist.genre} genre`);

    if (artist.language) {
      criteria.push(`${artist.language} language`);
    } else if (selectedLanguages.length > 0) {
      const langs = selectedLanguages.map(id => languageNames[id]).filter(Boolean);
      if (langs.length > 0) criteria.push(langs.join('/') + ' language');
    }

    if (selectedYears) criteria.push(`${selectedYears.from}-${selectedYears.to} era`);

    return criteria.length > 0 ? `Showing because: ${criteria.join(' • ')}` : 'Matched your preferences';
  };

  const selectedArtistObjects = artists.filter(a => selectedArtistIds.includes(a.id));
  const canContinue = selectedArtistIds.length >= 1;
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 p-6">
      <div className="max-w-7xl mx-auto py-12">
        ...
        <div className="text-center mb-12 space-y-4">
          ...
          <p className="text-white/60">Selected: {selectedArtistIds.length}</p>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-12">
            {artists.map(artist => (
              <button
                ...
                className={`relative group overflow-hidden rounded-2xl transition-all duration-300 aspect-square ${selectedArtistIds.includes(artist.id)
              ...
              >
                <div className="absolute inset-0">
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-t ${selectedArtistIds.includes(artist.id)
        ...
        <div className="flex gap-4 max-w-2xl mx-auto">
          ...
          <button
            onClick={() => onSkip(selectedArtistObjects)}
          ...
          <button
            onClick={() => onContinue(selectedArtistObjects)}
