...
const GenreSelection = ({ initialSelected = [], onContinue, onSkip, onBack }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-900 via-purple-900 to-fuchsia-900 p-6">
      <div className="max-w-6xl mx-auto py-12">
        ...
        <div className="text-center mb-12 space-y-4">
          <h2 className="text-5xl font-bold text-white">What music genres do you love?</h2>
          <p className="text-xl text-white/80">Select at least 1 favorite genre</p>
          <div className="flex items-center justify-center gap-4">
            <p className="text-white/60">Selected: {selectedGenres.length}</p>
            {selectedGenres.length >= 1 && <p className="text-green-300 text-sm">✓ Good to go!</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-12">
          {genres.map((genre) => (
            <button
              ...
              className={`relative group overflow-hidden rounded-2xl transition-all duration-300 aspect-square ${
            ...
            >
              ...
              <div className="absolute inset-0 flex items-end justify-center p-4">
                <h3 className="text-white font-bold text-xl text-center drop-shadow-lg">{genre.name}</h3>
              </div>
        ...
        {!canContinue && selectedGenres.length === 0 && (
          <p className="text-center text-white/60 text-sm mt-4">Select at least 1 genre to continue</p>
        )}
      </div>
    </div>
  );
