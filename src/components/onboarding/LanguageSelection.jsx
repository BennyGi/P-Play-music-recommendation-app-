...
const LanguageSelection = ({ initialSelected = [], onContinue, onSkip, onBack }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 p-6">
      <div className="max-w-7xl mx-auto py-12">
        ...
        <div className="text-center mb-12 space-y-4">
          ...
          <h2 className="text-5xl font-bold text-white">What languages do you listen to?</h2>
          <p className="text-xl text-white/80">Select at least one language • 50 languages available</p>
          <div className="flex items-center justify-center gap-4">
            <p className="text-white/60">Selected: {selectedLanguages.length}</p>
            {selectedLanguages.length >= 1 && <p className="text-green-300 text-sm">✓ Good to go!</p>}
          </div>
        </div>
        ...
        {!canContinue && selectedLanguages.length === 0 && (
          <p className="text-center text-white/60 text-sm mt-4">Select at least 1 language to continue</p>
        )}
      </div>
    </div>
  );
