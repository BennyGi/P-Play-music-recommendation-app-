// FILE: src/components/onboarding/LandingScreen.jsx
import React from 'react';
import { Music, LogIn, UserPlus, Library } from 'lucide-react';
import { StorageService } from '../../utils/storage';

const LandingScreen = ({ onLoginClick, onSignupClick, onJumpToLibraryClick }) => {
  const showJumpToLibrary = StorageService.hasAnyStoredLibraryPlaylists();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white/10 backdrop-blur-xl rounded-3xl p-10 border border-white/20 text-center shadow-2xl">
        ...
        <h1 className="text-5xl font-black text-white mb-4 tracking-tight">P-Play</h1>
        ...
        <div className="space-y-4">
          {showJumpToLibrary && (
            <button
              onClick={onJumpToLibraryClick}
              className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold py-4 rounded-xl hover:opacity-90 transition-all transform hover:scale-105 flex items-center justify-center gap-3 shadow-lg"
            >
        ...
        <p className="mt-8 text-white/40 text-sm">Version 1.0.3 - Music Discovery</p>
      </div>
    </div>
  );
};
